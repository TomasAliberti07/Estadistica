...
def obtener_matriz_y_resultados():
    try:
        ...
        for fila in matriz:
            if all(element == 0 for element in fila):
                messagebox.showerror("Error", "Una fila no puede ser nula (todos los elementos son cero).")
                return
        ...
        if soluciones is not None:
            ...
            for i in range(n):
                mensaje += f"x{chr(0x2081)} = {round(soluciones[i], 4)}\n"
...
def crear_entradas():
    try:
        ...
        for i in range(n):
            fila = []
            for j in range(n):
                label = tk.Label(frame_matriz, text=f"a{chr(0x2081)}{chr(0x2081 + j)}:",fg="black")
            ...
            label_resultado = tk.Label(frame_matriz, text=f"b{chr(0x2081 + i)}:",fg="black")
...
label_n = tk.Label(root, text="Tamaño de la matriz (nxn) Debe ser cuadrada:", bg="#D3D3D3",fg="black")
...
frame_matriz = tk.Frame(root, bg="#D3D3D3")
